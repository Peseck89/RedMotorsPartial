({
  init: function (component, event, helper) {
    ...
    var action = component.get("c.obtenerAssets");
    ...
    action.setCallback(this, function (response) {
      ...
      if (state === "SUCCESS") {
        var storeResponse = response.getReturnValue();
        // console.log("respuesta obtenerAssets", storeResponse);
        var lstAssets = response.getReturnValue();
        component.set("v.assets", lstAssets);
    ...
    var secondAction = component.get("c.getSeguroDetails");
    secondAction.setCallback(this, function (response) {
      var state = response.getState();
      console.log("State Second Action");
      console.log(state);
      if (state === "SUCCESS") {
        var storeResponse = response.getReturnValue();
        console.log("respuesta obtenerSeguros", storeResponse);
        // var lstAssets = response.getReturnValue();
        var lstAssets = storeResponse.map(function (item) {
          var newItem = JSON.parse(JSON.stringify(item));
          if (newItem.seguros.length === 0) {
            newItem.seguros.push({
              Tipo_seguro__c: "Sin seguro activo",
            });
          }
          return newItem;
        });
        component.set("v.seguros", lstAssets);
        component.set("v.assets", lstAssets);
      } else {
        console.log("error");
        console.log(response);
  ...
  handleOpenDesasignar: function(component, event, helper) {
    component.set('v.quitarVehiculo', true)
    var vehiculoId = event.currentTarget.getAttribute("data-id");
    component.set('v.vehiculoId', vehiculoId)
    console.log('Vehiculo ID', vehiculoId);
    
  },

  reload: function(component, event, helper) {
    component.set('v.quitarVehiculo', false)
  },

  handleDesasignar: function (component, event, helper) {
    console.log("Entro a desasignar");
    // var vehiculoId = event.currentTarget.getAttribute("data-id");
    var vehiculoId = component.get("v.vehiculoId");
    console.log("CUrrent vehiculo");
    console.log(vehiculoId);
    component.set("v.isLoading", true);
    var motivoBaja = component.get("v.motivoBaja");
    // var motivoBaja = "Prueba";
    ...
    action.setCallback(this, function (response) {
      ...
      if (state === "SUCCESS") {
        var storeResponse = response.getReturnValue();
        console.log(storeResponse);
        alert("Se ha solicitado la desasignacion de manera correcta.");
        component.set('v.quitarVehiculo', false)
        location.reload();
      } else {
        console.log("error");
        console.log(response);
        console.log(response.getError()[0].message);
        alert("Ha ocurrido un error, vuelva a intentarlo más tarde o comuníquese con nosotros en el botón de Asistencia en Línea");
      }
    });
    $A.enqueueAction(action);
  },
  ...
  handleOnChangeSelectedMarca: function (component, event, helper) {
    ...
    if (marcaSelected != marcaUrlAnt) {
      window.location.replace(
        "/s/communitycarsmanagement?servicio=" + marcaSelected
      );
    }
