({
    doInit: function(component, event, helper) {
        ...
        var marcaUrl = urlParams.get('marca') || 'Motorrad'; // Valor por defecto
        var marcaDisplay = marcaUrl == 'Motorrad' ? 'BMW Motorrad' : marcaUrl;
        
        component.set("v.marcaSelected", marcaUrl);
        component.set("v.marcaDisplay", marcaDisplay);

        var action = component.get("c.getFamilia");
        // action.setParams({
        //     marca: 'Motorrad'
        // });
        action.setParams({ marca: marcaUrl });
    ...
    navigateToComponent2: function(component, event, helper) {
        ...
        createTrafico.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                console.log(storeResponse);
                component.set("v.isLoading", false);
                window.location.replace('/walkingmoto/s/thankspagemotorrad?sucursal=uruca&marca='+marca);
            }else{
                component.set("v.isLoading", false);
            }
        });
