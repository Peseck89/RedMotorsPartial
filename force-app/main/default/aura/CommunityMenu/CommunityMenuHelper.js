({
  ...
  checkHourInOperativeHour: function (hour,formateDate) {
    ...
    console.log('==========================checkRecurrencias Process==================================');
  ...
  defineDataStructuresForServiceCenters: function (listOfEvents) {
    ...
    adaptListOfEventsToOppHours: function (listOfEvents) {
      ...
      //console.log(horasList); ESTE PROCESO CAMBIA LAS HORAS INICIALES Y FINALES A UNO QUE ESTE DENTRO DEL RANGO DE HORARIO DE SERVICIO
  ...
  createAvailabilityByDate: function (serviceCenterData) { // Agrega Horas Disponibles a cada día y Crea lista de citas x día
    ...
    for(var j = 0; j <= eventSize2; j++){
      if(lOfEvents[j][1] == '6:30 AM' && lOfEvents[j][2] == '7:30 AM' ){
    ...
    for(var k = 0; k <= eventSize2; k++){
      if(lOfEvents[k][1] == '7:30 AM' && lOfEvents[k][2] == '7:30 AM' ){
  ...
  createNewCalendarEvent: function (day, hour, component) {
    ...
    if(ejemplo == true){
      ...
      description = component.get("v.additionalSubService");
    ...
    //================================= ESTO VALIDA QUE SI ESTE LIBRE ==================================
      ...
      var action2 = component.get("c.callExternalApiOcupied");
      action2.setParams({
        servicio: serviceCenter,
        sucursal: serviceCenter,
        fecha: new Date(startDateHour).toISOString(),
      });
  ...
  myFunction2: function (component, event, helper) {
    ...
    function createCalendar(date, side) {//correcto crea calendario}
      ...
      if (side == "left") {
        ...
        var action = component.get("c.callExternalApiDates");
        console.log('selectedTabServiceCenter');
        console.log(selectedTabServiceCenter);
        console.log('formattedDate');
        console.log(formattedDate);
        // Set up the callback
        action.setParams({       
          sucursal: selectedTabServiceCenter,
          fecha: formattedDate
        });
        action.setCallback(this, function(response) {
          var state = response.getState();
          if(state === "SUCCESS"){
            component.set("v.isLoading", true);
            document.getElementById('centroDeServicio2').style.display = "none";
            ...
            for (let i = 1; i <= lastDay; i++) {
              ...
              if (component.get("v.controllerFirstRun") != true) {
                ...
                console.log('************************************ Nuevo proceso para rojos y verdes dia en el ciclo ************************************');            
                console.log(yourDate);
                console.log(opciones);
                console.log(latestDate);
                let isYourDateInOpciones = opciones.includes(yourDate);
                
                let isYourDateLessThanOrEqualToLatest = new Date(yourDate) <= new Date(latestDate);
                    ...
                    for (let n = 0; n < keys.length; n++) {
                      ...
                      for (let k = 0; k < mapaEventoDia.length; k++) {
                        ...
                        for (let j = 0; j < result.length; j++) {     
                          var time1 = key;
                            var time2 = result[j];
                            var normalizedTime1 = helper.normalizeTime(time1);
                            var normalizedTime2 = helper.normalizeTime(time2);

                          if(sortedData.hasOwnProperty(key) && normalizedTime1 == normalizedTime2){                    
                ...
                var classColorAdapted = '';
                // Final check

                let today = new Date();  
                if(latestDate == undefined &&  (new Date(yourDate) > yesterday) ){
                  if (
                    new Date(yourDate).getMonth() === today.getMonth() &&
                    new Date(yourDate).getFullYear() === today.getFullYear()
                  ){
                    isYourDateInOpciones = false;
                ...
                if (isYourDateInOpciones && isYourDateLessThanOrEqualToLatest  ) {
                   ...
                   classColorAdapted = 'circle-red-availability';
                    console.log('Your date does not meet the conditions.');
                }else if(!isYourDateLessThanOrEqualToLatest){
                   classColorAdapted = 'circle-white-availability';
                }
        ...
        // Enqueue the action
        $A.enqueueAction(action);
        //en este se pondra el ciclo para llamada
    ...
    function showEvents() {
      ...
      console.log(formattedDate);
      // 04/10/2023
      var formattedDateComm =  String(helper.selectedDate.getDate()).padStart(2, '0') + '/' + String(helper.selectedDate.getMonth() + 1).padStart(2, '0') + '/' +  helper.selectedDate.getFullYear();

      let sidebarEvents = document.getElementById("sidebarEvents");
      var serviceCenterId = component.get("v.selectedSucursal").toString();
      var servicioSeleccionado =  component.get("v.serviceSelected");
      // 
      // console.log( asesoresTotales);
      // console.log(mecanicos);        
      if(true){
        ...
        //Aqui se deberia hacer el llamado: formattedDateComm
        console.log('=========================================== NUEVO PROCESO DE LLAMADA ===============================');
        console.log(formattedDateComm);
        console.log(serviceCenterId);
        ...
        action.setParams({
          ...
          sucursal: serviceCenterId,
          fecha: formattedDateComm
        });
        action.setCallback(this, function(response) {
            var state = response.getState();
            ...
            if (state === "SUCCESS") {
                ...
                if (true) {
                  ...
                  if (true) {
                    ...
                    for (var i = 0 ; i < 1 ; i++) {
                      ...
                      //Aqui revisar si la lista devuelta es mayor a 0 eventos
                      ...
                      // Iterate over each element in `horas`
                      for (var key in horas) {
                    
                        if (horas.hasOwnProperty(key)) {
                          var eventTime = horas[key];
                          // Skip "Otras fechas"
                          if (eventTime === "Otras fechas") {
                            continue;
                          }
                          // Format the time to 12-hour format
                          var formattedTime = formatTo12Hour(eventTime);
                          // Check if this time should be included (replace `toBeIncluded` with your actual condition if needed)
                          ...
                          if (toBeIncluded) {
                            ...
                            // Create the button with the time
                            ...
                            eventActionExecution.appendChild(document.createTextNode(formattedTime));
                            
                            // Append button to action div, action div to container, and container to sidebar
                            eventAction.appendChild(eventActionExecution);
                            eventContainer.appendChild(eventAction);
                            sidebarEvents.appendChild(eventContainer);
                            
                            // Increment events count if needed
                            eventsCount++;
                          }
                        }
                      }
              ...
              component.set("v.isLoading", false);
                ...
                if (errors && errors[0] && errors[0].message) {
                  ...
                  console.log('entro showEvents Else');
                  let sidebarTitle = document.getElementById("sidebar-texttitle");
                  sidebarTitle.innerHTML = '0 Citas Disponibles';
                  let emptyMessage = document.createElement("div");
                  emptyMessage.className = "empty-message";
                  emptyMessage.innerHTML = "Lo sentimos, no hay citas disponibles en la fecha seleccionada.";
                  sidebarEvents.appendChild(emptyMessage);
    ...
    gridTable.onclick = function (e) {
      ...
      if (!e.target.innerHTML.includes('circle-gray-availability') && !e.target.innerHTML.includes('circle-red-availability')) {
  ...
  normalizeTime : function(timeString) {
    // Convert to uppercase and remove extra spaces and dots
    timeString = timeString.toUpperCase().replace(/\s/g, '').replace(/\./g, '');
    ...
    if (match) {
        ...
        // Return the normalized time as a string in 24-hour format
        return `${hours.toString().padStart(2, '0')}:${minutes}`;
    }

    return null; // Invalid time string
  },
  // Helper function to make the API call and render results
  fetchAvailableDates: function(component, selectedTabServiceCenter, date) {
    console.log('fetchAvailableDates called with:', date);

    component.set("v.isLoading", true);

    let action = component.get("c.callExternalApiDates");
    action.setParams({
        sucursal: selectedTabServiceCenter,
        fecha: date
    });
    ...
    action.setCallback(this, (response) => {
        ...
        if (state === "SUCCESS") {
            try {
                ...
                let sidebarEvents = document.getElementById("sidebarDiponibilidades");
                sidebarEvents.innerHTML = "";

                data.opciones.forEach(eventTime => {
                    if (eventTime === "Otra sucursal" || eventTime === "Otras fechas") return;
                    ...
                    eventActionExecution.type = "button";
                    eventActionExecution.className = "waves-effect waves-light btn blue lighten-2";
                    eventActionExecution.appendChild(document.createTextNode(eventTime));
    ...
    $A.enqueueAction(action);
    console.log("Action enqueued with date:", date);
  }
})