...
import saveQuantities from '@salesforce/apex/WoliGridController.saveQuantities';
import requestDespacho from '@salesforce/apex/WoliGridController.requestDespacho';
import requestApartar from '@salesforce/apex/WoliGridController.requestApartar';
import consultarDisponibilidad from '@salesforce/apex/WoliGridController.consultarDisponibilidad';
import requestDevolucion from '@salesforce/apex/WoliGridController.requestDevolucion';
import sendPDF from '@salesforce/apex/WoliGridController.sendPDF';
...
import Enable_Solicitud_de_Compra__c from '@salesforce/schema/User.Enable_Solicitud_de_Compra__c'; 
...
export default class WoliGridDespacho extends NavigationMixin(LightningElement) {
  @api recordId;
  @track lines = [];
  @track reservadas = [];
  @track pendientes = [];
  ...
  cols = COLS;
  reservadaCols;
  iframeSrc = "/apex/AlbaranPDFGroupTc?Id=";
  emailPDF;
  userId = Id;
  ...
  @wire(getRecord, {
    recordId: Id,
    fields: [CanDeleteWolis__c, Enable_Solicitud_de_Compra__c]
  })
  userDetails({ error, data }) {
    if (data) {
      this.CanDeleteWolis__c = data.fields.CanDeleteWolis__c.value;
      this.Enable_Solicitud_de_Compra__c = data.fields.Enable_Solicitud_de_Compra__c.value;
    } else if (error) {
      this.error = error;
    }
  }

  @wire(getRecord, { recordId: "$recordId", fields: [WorkOrderNumber] })
  ...
  connectedCallback() {
    ...
    loadStyle(this, lwcDatatableStyle)
      .then(() => {
        console.log("Loaded Successfully despachoLWCdataTable");
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  addDevolucionColumn() {
    const newColumn = {
      ...
      editable: { fieldName: "updateDevolucion" },
      type: "number",
  ...
  filterByEstadoReserva(data, estadoToFilter) {
    ...
    /*if(estadoToFilter === 'Pendiente de reserva'){
      return data.filter((item) => (item.estadoReserva === 'Pendiente de reserva' && item.aprobadoService === 'Aprobado' && (!item.estadoSolicitudCompra||item.estadoSolicitudCompra === 'Recibido en bodega')));
    }*/
  ...
  handleRowReservadasSelection(event) {
    ...
    event.detail.selectedRows.forEach((selectedRow) => {
      if (['Entregado', 'Sin Entregar'].includes(selectedRow.estadoEntrega)) {
  ...
  handleGetWolis() {
    ...
    getWolis({ listTosave: JSON.stringify([]), workorder: this.recordId })
      .then((result) => {
        ...
        this.showPendienteButtons =
          this.filterByEstadoReserva(
            JSON.parse(JSON.stringify(result)),
            "Pendiente de reserva"
          ).length > 0;

        this.showReservaButtons =
          this.filterByEstadoReserva(
            JSON.parse(JSON.stringify(result)),
            "Reservado"
          ).length > 0;
  ...
  handleDeleteWolis() {
    if(this.getUserPermissionToDelete()){
      this.isLoaded = true;
      deleteWolis({ workorderIds: JSON.stringify(this.pendientesSelectedRows) })
        .then((result) => {
          this.setMessageShow(
            "Success",
        ...
        result.forEach((ele) => {
          if (!ele.available) {
            ...
            if(ele.format === "slds-theme_error"){
              ...
              this.showSCbutton = this.Enable_Solicitud_de_Compra__c;
  ...
  handlePrintDespacho() {
    ...
    sendPDF({ workorder: this.recordId, email: this.emailPDF })
      .then((result) => {
  ...
  setFacturadaStyle(result) {
    result.forEach((ele) => {
      ele.isOpen = ele.estadoReserva !== 'Reservado';
      if (ele.isClosed) {
        ele.format = ele.isClosed
          ? "slds-theme_shade slds-theme_alert-texture"
          : ele.format;
      } else if(ele.estadoEntrega.includes("devolución")){
          ele.updateDevolucion = false;
          ele.format = "slds-theme_shade slds-theme_alert-texture";

      } else {
        ele.updateDevolucion = true;
