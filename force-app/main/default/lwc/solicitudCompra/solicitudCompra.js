...
import setPublicFile from '@salesforce/apex/QuoliGridController.setPublicFile';
import getDepartamentos from '@salesforce/apex/QuoliGridController.getDepartamentos';
import sendSC from '@salesforce/apex/WoliGridController.sendSC';
import { getRecord, getFieldValue } from 'lightning/uiRecordApi';
import quoteAccount from '@salesforce/schema/Quote.Account.Name';
import quoteName from '@salesforce/schema/Quote.Name';

const QUOTE_FIELDS = [quoteAccount, quoteName ];

export default class SolicitudCompra extends LightningElement {
    value = 'inProgress';
    @api recordId;
    @api accountId;
    @api rows;
    ...
    wiredAccount({ error, data }) {
        if (data) {
          ...
          this.pedido = data.fields.Name.value;
        } else if (error) {
    ...
    handlesetPublicFile(contentIds, uploadedFileNames) {
        setPublicFile({ contentIds: contentIds, quoteId: this.recordId })
          .then((result) => {
            this.dispatchEvent(
                new ShowToastEvent({
                  title: "Success",
      ...
      handleSendSC() {
        console.log('DEPARTAMENTO ENVIADO');
        console.log(this.department);
        this.showSendSCButton = true;
        sendSC({departamento: this.department, prioridad: this.priority, anticipo: this.anticipo, quoteId:this.recordId, idsToSC: this.solicitudSelectedRows})
          .then((result) => {
            if(result==='00'){
