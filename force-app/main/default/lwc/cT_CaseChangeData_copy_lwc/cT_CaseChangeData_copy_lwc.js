...
import cambiarFacturacionCaso from "@salesforce/apex/cT_ChangeData_copy_ctrl.cambiarFacturacionCaso";
import updateAssetkilometraje from "@salesforce/apex/cT_ChangeData_copy_ctrl.updateAssetkilometraje";
import updateFacturacionCaso from "@salesforce/apex/cT_ChangeData_copy_ctrl.updateFacturacionCaso";
import updateKilometraje from "@salesforce/apex/cT_ChangeData_copy_ctrl.updateKilometraje";
import getHistorialVehiculo from "@salesforce/apex/cT_ChangeData_copy_ctrl.getHistorialVehiculo";
import isCurrentUserAdmin from "@salesforce/apex/cT_ChangeData_copy_ctrl.isCurrentUserAdmin";
...
import updateAssetAddress from '@salesforce/apex/cT_ChangeData_copy_ctrl.updateAssetAddress';
...
export default class CT_CaseChangeData_copy_lwc extends LightningElement {
  @api recordId;
  ...
  @track isMotorcycle = false;
  @track isAtv = false;
  @track provincias = [];
  @track cantones = [];
  @track distritos = [];
  @track selectedProvinciaId;
  @track selectedCantonId;
  @track selectedDistritoId;
  @track actualizoDireccion;
  ...
  @wire(obtenerProvincias)
  wiredProvincias({ error, data }) {
    if (data) {
      this.provincias = data.map((prov) => ({
        label: prov.Name,
        value: prov.Id,
      }));
    } else if (error) {
  ...
  handleProvinciaChange(event) {
    this.selectedProvinciaId = event.detail.value;
    this.selectedCantonId = null;
    this.selectedDistritoId = null;
    this.cantones = [];
    this.distritos = [];

    obtenerCantonesPorProvincia({ provinciaId: this.selectedProvinciaId })
      .then((result) => {
        this.cantones = result.map((canton) => ({
          label: canton.Name,
          value: canton.Id,
        }));
      })
  ...
  handleCantonChange(event) {
    this.selectedCantonId = event.detail.value;
    this.selectedDistritoId = null;
    this.distritos = [];

    obtenerDistritosPorCanton({ cantonId: this.selectedCantonId })
      .then((result) => {
        this.distritos = result.map((distrito) => ({
          label: distrito.Name,
          value: distrito.Id,
        }));
      })
  ...
  handleDistritoChange(event) {
    this.selectedDistritoId = event.detail.value;
  }
  ...
  saveAddress() {
    ...
    if (!this.selectedProvinciaId || !this.selectedCantonId || !this.selectedDistritoId) {
      this.showSpinner = false;
      this.dispatchEvent(
        new ShowToastEvent({
          title: 'Error',
          message: 'Por favor seleccione provincia, cantón y distrito',
          variant: 'error'
        })
        );
        return;
    ...
    const provincia = this.provincias.find(p => p.value === this.selectedProvinciaId);
    const canton = this.cantones.find(c => c.value === this.selectedCantonId);
    const distrito = this.distritos.find(d => d.value === this.selectedDistritoId);

    updateAssetAddress({
        assetId: this.IdAsset,
        caseId: this.recordId,
        state: provincia.label,
        city: canton.label,
        street: distrito.label
    })
    .then(() => {
        this.dispatchEvent(
            new ShowToastEvent({
    ...
    .catch(error => {
        ...
        this.dispatchEvent(
            new ShowToastEvent({
                title: 'Error',
                message: 'Error actualizando dirección: ' + error.body.message,
                variant: 'error'
            })
        );
    })
  ...
  getCaseData({ error, data }) {
    if (data) {
      ...
      this.actualizoDireccion = data[0].Actualizo_direccion__c;
      ...
      if (
        data[0].Asset.Tipo_de_veh_culo__c === "Motocicleta" ||
