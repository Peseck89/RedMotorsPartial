...
import recid from '@salesforce/schema/tiposDeTrabajoCaso__c.Caso__c';
import getWorkOrder from '@salesforce/apex/TipoTrabajoCasoController.getWorkOrder';
import getTipoTrabajo from '@salesforce/apex/TipoTrabajoCasoController.getTipoTrabajo';
import getAseguradora from '@salesforce/apex/TipoTrabajoCasoController.getAseguradora';
...
export default class CT_searchProductos extends NavigationMixin(LightningElement){
    idOrden = '';
    @api recId;
    cols = COLS;
    ...
    @wire(getTipoTrabajo, {trabajoId: '$recId'})
    wiredCase({ error, data }) {
        console.log('entro');
        if (data) {
            console.log(data[0]);
            this.record = data;
            this.casonuevo = this.record.Caso__c;
            this.tipotrabajonuevo = this.record.Tipotrabajo__c;
            this.tipodecargonuevo = this.record.Tipo_de_cargo__c;
            this.nombretrabajo = this.record.Nombre_TTC__c;
            console.log(this.record.Tipotrabajo__c);
    ...
    @wire(getAseguradora, {trabajoId: '$recId'})
    wiredWorko({ error, data }) {
        if (data) {
            if(data.workOrder){
                this.recordWO = data.workOrder;
            }

            if(data.trabajoCaso){
                this.aseguradoranuevo = data.trabajoCaso.RM_Aseguradora__c ? data.trabajoCaso.RM_Aseguradora__c : '';
                this.centronuevo = data.trabajoCaso.RM_CentroCosto__c ? data.trabajoCaso.RM_CentroCosto__c : '';
            }
        } else if (error) {
            console.log('Something went wrong:', error);
        }
    ...
    @api async handleSave2() {
        const allValid = [
                ...
                if (allValid) {
                    ...
                    console.log(this.tipotrabajonuevo);
                    console.log(this.productList_test);
                    saveLines({ listTosave: JSON.stringify(this.productList_test),workorder:this.idOrden, caseid:this.casonuevo, tipodetrabajoid:this.tipotrabajonuevo, tipodetrabajocaso:this.recordId, origin:"caseWorkTypes", tipodecargo:this.tipodecargonuevo, aseguradora:this.aseguradoranuevo ,centrodecosto:this.centronuevo, aliasGenerico:this.nombretrabajo})
                            .then((result) => {
                            this._title = 'Success!';
                            ...
                            .catch((error) => {
                                ...
                                this.dispatchEvent(new ShowToastEvent({
                                    title: 'Atención',
                                    message: 'Algo no salió bien, consulte su administrador',                                    
                                    variant: 'error',
                                }));      
    ...
    @wire(getWorkOrder, {caseId: '$recId'})
    ...
    searchProductAction() {
        ...
        parametersMO["workOrder"] = this.idOrden; // this.workOrderId
