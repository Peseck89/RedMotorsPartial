...
export default class Rm_ct_crear_tiempo_improductivo_combobox extends LightningModal {
    ...
    tipoValues({ error, data }) {
        if (data) {
            this.tipoOptions = data.map(type => {
                return {
                    label: type.label,
                    value: type.label
                };
    ...
    handleSave(){
        this.isLoading = true;
        crearActividad({mechanicId: this.mechanicId, territoryId: this.territoryId, tipo: this.tipoSeleccionado})
            .then(() => {
