...
## Corrección del riesgo de Record Type en `productJSON.cls` para RMPEKING (2026-07-29, undécima pasada)

Worktree: `C:\Users\dokur\Documents\Repositorios\RedMotors-Sprint1-ProductJSON-RecordType-Peking`
Rama: `feature/pc/redmotors-sprint1-productjson-recordtype-peking-20260729` (creada desde `feature/pc/redmotors-empresa-marcas-chinas-sprint1-20260724` @ `826bd7f`).

70. **Riesgo confirmado (ver punto 66):** en `productJSON.cls`, para productos vehículo `applyVehicleFields` asignaba `RecordTypeId = '0124U00000111EDQAY'` (Record Type `Vehiculos`) de forma incondicional para cualquier empresa; para productos no-vehículo `applyCoreProductFields` nunca asignaba `RecordTypeId`, cayendo en el default del org (también `Vehiculos`). Ninguno de los dos admite `RMPEKING` en `Empresa__c` (solo el Record Type `Producto Red Motors`/`Producto_Red_Motors` lo admite). Hoy esto no se manifiesta porque `validateCompanyCanProcessWarehouses` bloquea la creación **y** la actualización de productos RMPEKING antes de cualquier `insert`/`update` (confirmado: la llamada existe en ambas ramas, no solo en la de creación) — pero el riesgo quedaría activo en cuanto ese bloqueo se levante.
71. **Corrección aplicada (mínima, solo `productJSON.cls`):** se agregó `getProductoRedMotorsRecordTypeId()`, método privado que consulta `RecordType WHERE SObjectType='Product2' AND DeveloperName='Producto_Red_Motors'`, afirma que existe exactamente uno, lo cachea en una variable estática (resuelto una sola vez por ejecución, sin SOQL dentro de ningún loop) y lanza `ProductCompanyConfigurationException` de forma controlada si no existe (sin fallback silencioso a `Vehiculos`). Se usa en dos puntos:
    - `applyCoreProductFields`: si `w.empresa == 'RMPEKING'`, asigna `p.RecordTypeId` dinámicamente (cubre la ruta no-vehículo).
    - `applyVehicleFields`: el `RecordTypeId` ahora es condicional — `Producto_Red_Motors` dinámico para RMPEKING, y exactamente el mismo Id fijo `'0124U00000111EDQAY'` de siempre para cualquier otra empresa (comportamiento de `RMBAVARIAN`/`RMOTOBAI` sin cambios).
    - **No se usó ningún Id fijo para RMPEKING. No se tocó `precioProductoJSON.cls`. No se tocó metadata de Record Type/picklist. No se creó bodega. Se preservó exactamente el bloqueo de bodega principal para RMPEKING** (`validateCompanyCanProcessWarehouses` sin cambios).
72. **Pruebas agregadas/modificadas en `productJSONTest.cls`:**
    - `testRecordTypeProductoRedMotorsExisteParaResolucionDinamicaPeking` (nueva): reutiliza el mismo patrón de `precioProductoJSONTest.getProductoRedMotorsRecordTypeId()` (consulta por `DeveloperName`, `assertEquals(1, ...)`) para confirmar la precondición de la que depende la resolución dinámica.
    - `testVehiculoTransitoOtobai` y `testVehiculoUsadoValidacionCompleta` (modificadas, agregando aserciones): ahora verifican explícitamente que `Product2.RecordTypeId` de un producto RMOTOBAI (vehículo en tránsito) y de uno RMBAVARIAN (usado) sigue siendo exactamente `'0124U00000111EDQAY'` — prueba directa de que el comportamiento de Bavarian/Otobai no cambió.
    - `testPekingDetieneAntesDeCrearProductoSinBodegaConfirmada` (existente, sin cambios): sigue confirmando que RMPEKING se detiene con `CODIGO 500` antes de crear ningún producto — el bloqueo de bodega principal se conserva intacto tras esta corrección.
73. **Limitación E2E pendiente:** no es posible, sin modificar el bloqueo de bodega principal (prohibido en este bloque) ni crear datos reales en Partial (prohibido), ejecutar `productJSON.createProduct` de punta a punta con `empresa='RMPEKING'` y verificar el `RecordTypeId` resultante sobre un `Product2` real insertado — `validateCompanyCanProcessWarehouses` lanza la excepción controlada antes de llegar a `applyCoreProductFields`/`applyVehicleFields` en cualquier escenario. La cobertura funcional alcanzable hoy se limita a: (a) confirmar que la precondición (existencia del Record Type `Producto_Red_Motors`) se cumple, y (b) revisión de código confirmando que ningún Id fijo se usa para la rama RMPEKING. La verificación E2E completa (RecordTypeId real asignado a un Product2 RMPEKING insertado) queda pendiente hasta que exista una bodega principal oficial confirmada para RMPEKING (ver punto 17) que permita superar `validateCompanyCanProcessWarehouses` de forma legítima.
74. **Manifest creado:** `manifest/sprint1-productjson-recordtype-peking-validation.xml` (API 67.0), con exactamente 2 `ApexClass`: `productJSON` y `productJSONTest`.
75. **No se ejecutó Salesforce CLI, dry-run, deploy, DML ni Apex anónimo en este bloque.** No hubo commit ni push — solo cambios locales en el worktree aislado, pendientes de revisión.

## Fortalecimiento de pruebas de Record Type en `productJSON.cls` (2026-07-29, duodécima pasada)

76. **Motivo:** la prueba `testRecordTypeProductoRedMotorsExisteParaResolucionDinamicaPeking` del punto 72 solo confirmaba la precondición (existencia del Record Type en el org), no que el código productivo realmente lo usara. Se refactorizó para hacer observable la resolución real.
77. **Estructura final cambiada respecto al punto 71:** la lógica de `applyCoreProductFields`/`applyVehicleFields` ahora pasa por un único método `resolveProductRecordTypeId(String empresa, Boolean esVehiculo)`, marcado `@TestVisible`, que centraliza la decisión:
    - `empresa == 'RMPEKING'` → siempre `getProductoRedMotorsRecordTypeId()` (dinámico, sin Id fijo), independientemente de si es vehículo o no.
    - `empresa != 'RMPEKING' && esVehiculo == true` → Id fijo `'0124U00000111EDQAY'` (`Vehiculos`), exactamente como antes.
    - `empresa != 'RMPEKING' && esVehiculo == false` → `null` (ninguna asignación), exactamente como antes.
    - `getProductoRedMotorsRecordTypeId()` se conserva sin cambios (cacheada, una sola consulta por transacción, falla controlado si no existe el Record Type).
    - `applyCoreProductFields` solo asigna `p.RecordTypeId` cuando el resultado no es `null` (evita nulear `RecordTypeId` en actualizaciones de Bavarian/Otobai, que nunca seleccionan ese campo en la query previa al `update`).
78. **`@TestVisible` justificado exclusivamente en `resolveProductRecordTypeId`** (método puro, sin DML, sin colecciones): el bloqueo de bodega principal para RMPEKING impide observar el `insert` real en pruebas sin alterar comportamiento productivo; exponer este método puro permite comprobar directamente cuál `RecordTypeId` usaría producción. No se agregó `@TestVisible` a ninguna variable, colección ni método con DML.
79. **Pruebas nuevas en `productJSONTest.cls`** (reemplazan a la del punto 72, que solo probaba la precondición): `testResolverRMPekingVehiculoUsaProductoRedMotors`, `testResolverRMPekingNoVehiculoUsaProductoRedMotors`, `testResolverRMOtobaiVehiculoConservaVehiculos`, `testResolverRMBavarianVehiculoConservaVehiculos`, `testResolverEmpresaExistenteNoVehiculoConservaComportamientoPrevio` — todas invocan `productJSON.resolveProductRecordTypeId(...)` directamente y comparan contra el Id real esperado (consultado con el mismo patrón `DeveloperName`/`assertEquals(1,...)`), validando así el uso productivo real, no solo la existencia del Record Type.
80. **Escenario "Record Type faltante" — sin prueba automatizada, documentado:** no se puede simular sin eliminar metadata real (prohibido) ni introducir una abstracción artificial solo para pruebas (evitado deliberadamente). Cobertura de esta rama es por revisión de código: `getProductoRedMotorsRecordTypeId` lanza `ProductCompanyConfigurationException` de forma controlada, sin fallback a `Vehiculos`, cuando la consulta no encuentra el Record Type.
81. **`testVehiculoTransitoOtobai`, `testVehiculoUsadoValidacionCompleta` (punto 72) y `testPekingDetieneAntesDeCrearProductoSinBodegaConfirmada`:** sin cambios adicionales en esta pasada — siguen vigentes y pasando por la misma ruta E2E real (insert), confirmando que Bavarian/Otobai no cambiaron y que el bloqueo de bodega RMPEKING sigue intacto.
82. **El insert E2E de RMPEKING continúa pendiente** hasta disponer de una bodega principal oficial confirmada (ver punto 17) — no se levantó el bloqueo de bodega en esta pasada.
83. **Manifest sin cambios:** sigue siendo exactamente `productJSON` y `productJSONTest`. No se ejecutó Salesforce CLI, dry-run, deploy, DML ni Apex anónimo. No hubo commit ni push.

## Cierre del bloque — deploy exitoso a Partial (2026-07-29, decimotercera pasada)

84. **Dry-run exitoso:** `0AfAK000000xyHt0AI` — 2/2 componentes compilados, **12/12 pruebas aprobadas**, cobertura de `productJSON` **82%**, código de salida `0`. Sin deploy.
85. **Deploy exitoso a Partial:** `0AfAK000000xyJV0AY` — Target Org `peseck89@gmail.com.partial.redmotors`. 2/2 componentes, 12/12 pruebas, cobertura 82%, código de salida `0`. **Producción no fue tocada** — el deploy fue únicamente contra la sandbox Partial.
86. **Resumen final del bloque de corrección de Record Type en `productJSON.cls` (ya desplegado en Partial):**
    - RMPEKING utiliza dinámicamente el Record Type `Product2` con `DeveloperName = Producto_Red_Motors`, resuelto vía `resolveProductRecordTypeId`/`getProductoRedMotorsRecordTypeId` (ver puntos 71 y 77). **No se utilizó ningún Id fijo para RMPEKING.**
    - `RMBAVARIAN`/`RMOTOBAI` conservan el Record Type `Vehiculos` (`'0124U00000111EDQAY'`) existente para productos vehículo, sin cambios.
    - Productos no-vehículo de `RMBAVARIAN`/`RMOTOBAI` conservan el comportamiento previo (`RecordTypeId` sin asignar).
    - **El bloqueo de bodega principal para RMPEKING continúa intacto** (`validateCompanyCanProcessWarehouses` sin cambios).
    - **El insert E2E de RMPEKING continúa pendiente** hasta disponer de una bodega principal oficial (ver punto 17 y punto 73).
    - **Producción no fue tocada** en ningún momento de este bloque.
