...
## Diseño documentado, NO implementado (`precioProductoJSON` — pendiente de negocio)

3. **`precioProductoJSON`: Pricebook local `PEKING Local`, Pricebook dólares `PEKING Dólares` — confirmados por Diego, pero no implementados en código.** `precioProductoJSON.cls` fue restaurado a `HEAD` (ver nota de limpieza final arriba): **no** contiene ninguna rama para `RMPEKING`. La razón es que el identificador de producto que debe usar PEKING (equivalente a `Codigo_de_Producto__c` en Bavarian o `CodigoProductoInterno__c` en Otobai) **no está confirmado** (ver punto 5), y no se debe implementar solo la mitad del flujo (Pricebooks sin identificador de producto) de forma no probada y fuera del manifest de validación. Cuando Diego confirme el identificador, se debe implementar la rama completa (identificación de producto + selección de Pricebook) en un bloque propio, con sus tests.
...
5. ~~Identificador de producto para PEKING = `CodigoProductoInterno__c = articulo + '-' + empresa`~~. **Revertido.** Se había replicado el patrón de RMOTOBAI sin confirmación de Diego. La reunión solo confirmó código de empresa y nombres de Pricebook, **no** el campo identificador de producto. Queda como pregunta abierta para Diego (ver punto 18).
...
18. **Campo identificador del producto PEKING en `precioProductoJSON`** (ver punto 5) — no se debe repetir la pregunta sobre RMPEKING, endpoints, los seis catálogos, Pricebooks u Omoda/Jaecoo, ya respondidas.
