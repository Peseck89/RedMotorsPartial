...
## Actualización (2026-07-30, misma jornada) — bloqueo de Pricebook2–Empresa__c resuelto

Luis autorizó expresamente crear `Pricebook2.Empresa__c` (Lookup a `Empresa__c`, no obligatorio), desbloqueando la implementación de los 5 Flows de Pricebook (`Opportunity_Flow`, `Opp_flow_v4`, `BMW_ImportarPlantilla`, `BMW_Gestiona_Listas_de_Precios`, `Obtener_PricebookEntry_en_Linea_de_Plantilla_de_Presupuesto`) y de `rm_vu_inventario`. Esto constituye la autorización nombre-por-nombre y cambio-solicitado que la Fase 2/Fase 3 de este documento marcaba como pendiente para `BMW_Gestiona_Listas_de_Precios`, `Obtener_PricebookEntry_en_Linea_de_Plantilla_de_Presupuesto` y `rm_vu_inventario` — el mandato de la tarea de esta jornada, recibido directamente de Luis, especifica el cambio exacto requerido para cada uno (sección "Flows confirmados" / "rm_vu_inventario" del mandato). Todos los 5 Flows y `rm_vu_inventario` quedan **AUTORIZADO Y CONFIRMADO** e implementados. Los 12 Flows candidatos adicionales (sección "Candidatos mantenidos fuera del alcance confirmado" arriba) **permanecen sin autorización** y no fueron tocados. Detalle completo, evidencia de deploy y QA funcional en `CIERRE_TECNICO_SPRINT2_EMPRESA_PRICEBOOK.md`.

## Reconciliación (2026-08-12) — `Opp_Flow_V5` y remediaciones posteriores

Esta sección reconcilia el estado histórico de `Opp_Flow_V5` sin flexibilizar la regla principal de autorización de este documento ni reinterpretar una ejecución previa como permiso abierto para nuevos cambios.

| Hito | Evidencia comprobada | Consecuencia de alcance |
|---|---|---|
| Candidato inicialmente no autorizado | La actualización del 2026-07-30 registró `Opp_Flow_V5` entre los 12 candidatos adicionales encontrados por análisis técnico, sin autorización explícita por nombre para implementación. | Su presencia en inventarios, matrices o metadata no lo autorizaba por sí sola. |
| Decisiones técnicas de Luis | El commit documental `6e160a6` (*incorporate flow decisions from Luis*, 2026-08-04) registra para el sublote 2D2 que `Opp_Flow_V5` debía partir exclusivamente de la versión activa v29, ignorar el Draft v30 y presentar selección explícita de Empresa. | Quedaron resueltas la versión base y la decisión técnica/UX; el propio documento conservó la condición de que cada sublote requería autorización específica de ejecución. |
