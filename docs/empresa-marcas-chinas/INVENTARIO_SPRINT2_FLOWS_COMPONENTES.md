...
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | `Opportunity_Flow` | PDF §4 (fila "Flow / Opportunity_Flow, Opp_flow_v4") | Opportunity | Decision `Encuentra_Price_Book` + Assignments `Asigna_Bavarial_Local/Dolar`, `Asigna_Oto_Local/Otobai_Dolar` | Agregar 2 reglas + 2 assignments para la compañía nueva (Local/Dólar) | Sí | Active / identical | No documentado en fuentes disponibles | No confirmado — candidato | Pendiente de análisis funcional propio de Sprint 2 |
| 2 | `Opp_flow_v4` | PDF §4 (misma fila que #1) | Opportunity | Mismo patrón que `Opportunity_Flow` | Mismo cambio que `Opportunity_Flow` | Sí | Active / identical | No documentado | No confirmado — candidato | Pendiente |
| 3 | `BMW_ImportarPlantilla` | PDF §4 + §10 | Plantilla_de_Presupuesto\_\_c / WorkOrder | Fórmula `EmpresaProductoMantenimiento`: `IF(CONTAINS(Pricebook.Name,"Otobai"),"RMOTOBAI","RMBAVARIAN")`; RecordLookup filtra `Plantilla_de_Presupuesto__c.BMW_Compania__c` | Pasar el `else` binario a 3 vías; crear plantillas de presupuesto para la compañía nueva | Sí | Active / identical | No documentado | No confirmado — candidato | Pendiente |
