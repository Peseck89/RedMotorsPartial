...
- **Remediación técnica `Opp_Flow_V5` (2026-08-12):** corregida exclusivamente la ruta Taller mediante una pantalla mínima con lookup estructural de Empresa; la selección específica de Taller tiene precedencia y las demás rutas conservan la selección general. `Opp_Flow_V5` v31 quedó activa en Partial. El primer QA real confirmó la entrada correcta a la pantalla y detectó un bloqueo externo al Flow por acceso a `Empresa__c` y layouts; se desplegó un Permission Set de solo lectura y se agregaron únicamente `Autos` y `Taller Autos`. El bloqueo de configuración quedó resuelto; QA funcional manual pendiente de repetición. El inventario mantiene 20 Flows: 8 con validación técnica OK/QA manual pendiente y 0 bloqueados técnicamente. Ver [`RESULTADO_REMEDIACION_OPP_FLOW_V5_TALLER_20260812.md`](RESULTADO_REMEDIACION_OPP_FLOW_V5_TALLER_20260812.md).
