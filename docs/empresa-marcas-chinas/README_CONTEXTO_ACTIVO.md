...
- **QA P0 — `Opp_flow_V3` (2026-08-12):** la ejecución funcional v29 creó y conservó Opportunity y Quote con PEKING, CRC y Pricebook `PEKING Local`; la creación queda en **QA OK**. El defecto posterior del componente legacy de navegación fue remediado de forma aislada con un enlace estándar; v30 está activa y queda pendiente únicamente validar el enlace integrado en una ejecución funcional normal. `Opp_Flow_v6` y `Opportunity_Flow_V2` siguen listos parcialmente; `PlanDeMantenimientoV2`, `CreateWoliFromExpense` y `AgregarManoObra` continúan esperando datos funcionales de entrada. Ver [`RESULTADO_QA_REMEDIACION_OPP_FLOW_V3_20260812.md`](RESULTADO_QA_REMEDIACION_OPP_FLOW_V3_20260812.md) y [`RESULTADO_PREPARACION_QA_P0_20260812.md`](RESULTADO_PREPARACION_QA_P0_20260812.md).
- **Remediación técnica `Opp_Flow_v6` (2026-08-12):** el QA v80 demostró un fault en `CreateQuote` porque `Quote.Compania__c` recibió `PEKING`, valor incompatible con el picklist legacy Bavarian/Otobai. Se cambió exclusivamente la fuente a `EmpresaLegacySeleccionada`, preservando Bavarian/Otobai y dejando vacío el campo legacy para PEKING. v81 está activa en Partial; Empresa estructural y resolver permanecen intactos. Estado: **VALIDACIÓN TÉCNICA OK — QA FUNCIONAL MANUAL PENDIENTE**. Ver [`RESULTADO_REMEDIACION_OPP_FLOW_V6_20260812.md`](RESULTADO_REMEDIACION_OPP_FLOW_V6_20260812.md).
