...
- **Preparación QA P0 (2026-08-12):** `Opp_flow_V3` quedó **LISTO PARA QA MANUAL** con usuario y datos QA existentes. `Opp_Flow_v6` y `Opportunity_Flow_V2` quedaron listos de forma parcial para su ruta Taller/general; sus rutas Mostrador requieren una sesión funcional autorizada de ese tipo. `PlanDeMantenimientoV2`, `CreateWoliFromExpense` y `AgregarManoObra` conservan validación técnica, pero no están listos para QA positivo porque no existen los registros funcionales de entrada necesarios (QuoteLineItem o Work Order/Expense PEKING). Ver [`RESULTADO_PREPARACION_QA_P0_20260812.md`](RESULTADO_PREPARACION_QA_P0_20260812.md).
